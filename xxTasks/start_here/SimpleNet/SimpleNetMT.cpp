...
static const std::string &OUTPUT_POISIONED_APPLE = INPUT_POISENED_APPLE;
...
{
	m_outputsQ->Enqueue(GetShutdownPoisionedApple());
}



SimpleNetMT::OutputsQ::value_type SimpleNetMT::GetConnectionPoisionedApple(SocketId a_socketId)
{
	return OutputsQ::value_type{a_socketId, OUTPUT_POISIONED_APPLE};
}

const SimpleNetMT::OutputsQ::value_type &SimpleNetMT::GetShutdownPoisionedApple()
{
	static OutputsQ::value_type s_poisioned{-1, OUTPUT_POISIONED_APPLE};
	return s_poisioned;
}
...
void SimpleNetMT::Pump()
{
	...
	while (true)
	{
		...
		{
			if (output == GetShutdownPoisionedApple())
			{
				return;
			}
			if (m_openConnections.find(output.first) != m_openConnections.end())
			{
				if (output.second == OUTPUT_POISIONED_APPLE)
				{
					m_pImpl->CloseConnection(output.first);
				}
				else
				{
...
SimpleNetMT::Connection::Connection(Connection&& a_other) noexcept
...
SimpleNetMT::Connection& SimpleNetMT::Connection::operator=(Connection&& a_other) noexcept
{
	m_socketId = a_other.m_socketId;
	m_inputsQ = std::move(a_other.m_inputsQ);
	m_outputsQ = std::move(a_other.m_outputsQ);
...
void SimpleNetMT::Connection::CloseConnection()
{
	m_outputsQ->Enqueue(GetConnectionPoisionedApple(m_socketId));
}
