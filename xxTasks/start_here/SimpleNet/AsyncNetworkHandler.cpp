	...
	while (true)
	{
		if (!m_totalSocketsReady)
		{
			...
			for (int i = 0 ; m_totalSocketsReady && i < FD_SETSIZE ; i++)
			{
				...
				if (FD_ISSET(socketInfo.Socket, &writeSet))
				{
					...
					assert(socketInfo.writeLen <= std::numeric_limits<int>::max());
					size_t sendBytes = send(socketInfo.Socket, socketInfo.writeBuffer, static_cast<int>(socketInfo.writeLen), 0);
		...
		for ( ; m_totalSocketsReady ; ++m_nextReadSocketToTraverse)
		{
			...
			if (FD_ISSET(socketInfo.Socket, &m_lastReadSet))
			{
				...
				int availableBytesForRead = static_cast<int>(READ_DATA_BUFSIZE - socketInfo.BytesRECV);
				assert(availableBytesForRead > 0);
				size_t recvBytes = recv(socketInfo.Socket, &socketInfo.readBuffer[socketInfo.BytesRECV], availableBytesForRead, 0);
				if (recvBytes == -1)
				{
					if (WSAGetLastError() != WSAEWOULDBLOCK)
					{
				...
				else
				{
					...
					size_t prevBytes = socketInfo.BytesRECV;
...
bool AsyncNetworkHandler::FillInputString(SOCKET_INFORMATION &socketInfo, size_t prevBytes, std::string *_input)
{
...
void AsyncNetworkHandler::PutOutput(size_t _socketid, const std::string &_output)
{
	...
	for (int i = 0 ; i < _output.size() ; ++i)
	{
		if (_output[i] == '\n')
		{
			assert((m_socketArray[_socketid]->writeLen + 2 <= sizeof(m_socketArray[_socketid]->writeBuffer)) && "Either send shorter output or set the constant WRITE_DATA_BUFSIZE to a higher value");
		...
		{
			assert((m_socketArray[_socketid]->writeLen + 1 <= sizeof(m_socketArray[_socketid]->writeBuffer)) && "Either send shorter output or set the constant WRITE_DATA_BUFSIZE to a higher value");
...
void AsyncNetworkHandler::CloseConnection(size_t _socketid)
{
	if (m_socketArray[_socketid])
	{
		FreeSocketInformation(_socketid);
...
void AsyncNetworkHandler::FreeSocketInformation(size_t a_index)
{
