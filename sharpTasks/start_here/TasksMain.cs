    ...
    class TasksMain
    {
        ...
        void Execute(string[] a_args)
        {
            ...
            while (true)
            {
                using (SimpleServer.SimpleServer.Connection connection = server.WaitForConnection())
                {
                    ...
                    using (IDisposable usernameScope = m_logger.BeginScope(new[] { new KeyValuePair<string, object>("username", username) }))
                    {
        ...
        private void DisplayActiveTasks(Authorization a_authorization, TasksManager a_tasksMgr, SimpleServer.SimpleServer.Connection a_connection)
        {
            try 
            {
                ...
                if (tasks.Length == 0)
                {
                    a_connection.WriteLine($"Hello {a_authorization.Username}, there are currently no tasks that require attention.");
                }
                else
                {
                    ...
                    foreach (Task t in tasks)
                    {
                        if (t.IsUrgent)
                        {
                            a_connection.WriteLine($"YES     {t.Description}");
                        }
                        else
                        {
                            a_connection.WriteLine($"NO      {t.Description}");
                        }
                    }
                }
            } 
